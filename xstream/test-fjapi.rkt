...
;; mapE tests
(check-equal? (mapE (λ (i) (add1 i)) '(11 12 13)) '(12 13 14))
(check-equal? (mapE (λ (i) (add1 i)) '()) '())
(check-equal? (mapE (λ (i) (add1 i)) '(11 no-evt 13)) '(12 no-evt 14))
...
;; filterE tests
(check-equal? (filterE (λ (e) (not e)) (list #t #f #t 'no-evt)) (list 'no-evt #f 'no-evt 'no-evt))
...
(check-equal? (andE (list #f #t 'no-evt #f #t)
                    (list #t #t #t #t #f))
              (list 'no-evt #t 'no-evt 'no-evt 'no-evt))

;; orE tests
(check-equal? (orE (list #f #t 'no-evt #f #t)
                   (list #t #t #t #f #f))
              (list #t #t 'no-evt 'no-evt #t))
...
;; liftB
(check-equal? (liftB (λ (v) (+ v 3)) (behavior 0 (list 2 3 4)))
              (behavior 3 (list 5 6 7)))
(check-equal? (liftB2 + (behavior 0 (list 1 2 3)) (behavior 10 (list 1 2 3)))
              (behavior 10 (list 2 4 6)))
(check-equal? (liftB (λ (t) (<= t 2)) (behavior 1 (list 0 5)))
              (behavior #t (list #t #f)))
(check-equal? (liftB (λ (t) (<= t 2)) (behavior 1 '())) (behavior #t '()))
